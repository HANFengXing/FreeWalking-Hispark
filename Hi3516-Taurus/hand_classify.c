...
#define MODEL_FILE_GESTURE    "/userdata/models/hand_classify/test_inst.wk" // darknet framework wk model
...
HI_S32 Yolo2HandDetectResnetClassifyCal(uintptr_t model, VIDEO_FRAME_INFO_S *srcFrm, VIDEO_FRAME_INFO_S *dstFrm)
{
    ...
    int ret;
    int num = 0;
    /*model 参数用于传递神经网络模型或配置的信息，以便在函数中使用。
    srcFrm 参数表示输入帧的信息，包括要处理的图像数据。
    dstFrm 参数表示输出帧的信息，用于将处理后的图像数据写入到指定的输出帧中。*/

    //将输入的 YUV 格式的视频帧转换为原始图像
