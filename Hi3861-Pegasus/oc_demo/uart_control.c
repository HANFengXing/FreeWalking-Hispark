...
#define IOT_GPIO_IDX_3 3//SDA
#define IOT_GPIO_IDX_4 4//SCL
...
char globalDataString[1024] = {0};
void TrafficLightAppOption(HiTrafficLightMode appOptionMode, HiControlModeType appOptionType,unsigned char *data)
{
    ...
    switch (GetKeyStatus(CURRENT_MODE)) {
        case TRAFFIC_CONTROL_MODE:
            TrafficLightStatusReport(TRAFFIC_CONTROL_MODE, SetupTrflControlModule,data);
            break;
        case TRAFFIC_AUTO_MODE:
            TrafficLightStatusReport(TRAFFIC_AUTO_MODE, SetupTrflAutoModule,data);
            break;
        case TRAFFIC_HUMAN_MODE:
            TrafficLightStatusReport(TRAFFIC_HUMAN_MODE, SetupTrflHumanModule,data);
            break;
        default:
            break;
    }
...
static void ReportTrafficLightMsg()
{
    switch (GetKeyStatus(CURRENT_MODE)) {
        case TRAFFIC_CONTROL_MODE:
...
{   printf("test here!!!");
    ...
    IoTGpioInit(IOT_GPIO_IDX_3);//初始化
    IoSetFunc(IOT_GPIO_IDX_3,6);//引脚复用
    IoTGpioInit(IOT_GPIO_IDX_4);//初始化
    IoSetFunc(IOT_GPIO_IDX_3,6);
...
/* 主动上报 */
    ...
    while (1) {
        ...
        if (len > 0) {
          ...
          for(int i=0;i<UART_BUFF_SIZE-7;i++)
            {
                if(uartReadBuff[i]=="{")
                {int j=i;
                int flag=0;
                char num1[4]={0},num2[4]={0},num3[4]={0},num4[4]={0};
                    while(uartReadBuff[j]!=",")
                    {   num1[flag]=uartReadBuff[j];
                        flag++;
                        j++;
                    }
                    int x1=0;//x1y1是左下角点，x2y2是右上角点
                    for(int m=0;m<flag;m++)
                    {
                        x1=x1+num1[m]*10^(flag-1-m);
                    }
                    j++;
                    flag=0;
                    while(uartReadBuff[j]!=",")
                    {   num2[flag]=uartReadBuff[j];
                        flag++;
                        j++;
                    }
                    int y1=0;
                    for(int m=0;m<flag;m++)
                    {
                        y1=y1+num2[m]*10^(flag-1-m);
                    }
                    j++;
                     flag=0;
                    while(uartReadBuff[j]!=",")
                    {   num3[flag]=uartReadBuff[j];
                        flag++;
                        j++;
                    }
                    int x2=0;
                    for(int m=0;m<flag;m++)
                    {
                        x2=x2+num3[m]*10^(flag-1-m);
                    }
                    j++;
                     flag=0;
                    while(uartReadBuff[j]!=",")
                    {   num4[flag]=uartReadBuff[j];
                        flag++;
                        j++;
                    }
                    int y2=0;
                ...
                if((uartReadBuff[i]=='o')&&(uartReadBuff[i+1]=='b')&&(uartReadBuff[i+2]=='j')&&(uartReadBuff[i+3]=='N')&&(uartReadBuff[i+7]=='0'))
                {          
